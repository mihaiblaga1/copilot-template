...
- **Title:** `[{FeatureName}] {Anti-pattern short name} — {file basename}` (e.g. `[TopUsers] N+1 query in getTopUsers action — top-users.ts`)
- **Source label:** `{featureLabel}-performance` (e.g. `top-users-performance`). Create it if it does not exist per the template's pre-creation steps.
- **Layer:** Always `Performance`.
- **Source line:** `Generated by performance-auditor on {today's date}`.
...
| Severity      | **Impact** (prefix) |
| Fix           | **Suggested Fix** |
| Category      | **Layer** (use `Performance`) |

## Step 5 — Summary

After all tickets are created, output the post-creation summary from the template, then append:
