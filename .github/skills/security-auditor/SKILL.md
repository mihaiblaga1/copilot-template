...
description: "Scans a feature for OWASP Top 10 violations and Next.js-specific security anti-patterns, then creates Linear tickets for every actionable finding. Checks: missing auth checks, unvalidated inputs, exposed secrets, insecure headers, CSRF gaps, injection risks, and more. Use when: security audit, security review, audit [feature] for security, OWASP, vulnerability scan, check for security issues, security tickets."
...
Create one ticket per Critical, High, and Medium finding. Group all Low findings into a single ticket titled `[{FeatureName}] Low-severity security hardening — {n} findings`.

### Security-specific overrides

- **Source label:** `{featureLabel}-security` (e.g. `payments-security`). Create it with color `#DC2626` if it does not exist.
- **Layer:** Always `Security`.
- **Source line:** `Generated by security-auditor on {today's date}`.

Map each FINDING field to the template's description sections:

| FINDING field             | Template section        |
| ------------------------- | ----------------------- |
| Anti-pattern              | Title (imperative form) |
| Violation                 | **What** + **Problem**  |
| File / Line(s)            | **Location**            |
| Severity + OWASP Category | **Impact**              |
| Fix                       | **Suggested Fix**       |
